...
def get_model(model_name, seq_len=None, bg_mode=None, alpha=1.0):
    """ Create model by name and the configuration parameter.
        ...
        Args:
            ...
            alpha (float, optional): Scaling factor for model filters.

        Returns:
            model (torch.nn.Module): Model with specified configuration
    """

    if model_name != 'TrackNetV5':
    ...
    if bg_mode == 'subtract':
        ...
        _in = seq_len * 3
    model = TrackNetV5(in_dim=_in, out_dim=seq_len, alpha=alpha)
